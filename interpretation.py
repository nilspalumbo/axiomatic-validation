...
def plot_attentions(sample, intermediates=None, path="img/attns_sample.pdf", **kwargs):
    ...
    plot_attentions(get_ax(0), attentions_layer_1[0], "Block 1, Head 1: Attention Scores")
    for i in range(1,5):
        plot_attentions(get_ax(i), attentions_layer_2[i-1], f"Block 2, Head {i}: Attention Scores")
...
def get_layer_1_score_fns(model=None, config=None, **kwargs):
    ...
    def plot_df(df, title, softmax=False):
        ...
        if softmax:
            return alt.Chart(df, **({"title": title + " (Softmax)"} if title is not None else {})).mark_rect(width=20, height=20).encode(
                alt.Y("Destination:N"),
                alt.X("Source:N"),
                alt.Color("Softmax Score"),
            ) 
        else:
            return alt.Chart(df, **({"title": title} if title is not None else {})).mark_rect(width=20, height=20).encode(
                alt.Y("Destination:N"),
                alt.X("Source:N"),
                alt.Color("Score"),
            ) 
...
def get_axiom_evaluators(get_prefix_equivalence_model_rhs=None, get_component_equivalence_model_rhs=None, get_equivalence_model_lhs=None, get_prefix_replaceability_model=None, get_component_replaceability_model=None, test=None, concrete_model=None, **outer_kwargs):
    def get_epsilon(lhs, rhs, inputs=test, allowable_deviation=0, return_match_rate=False, confidence_level=0.95):
        ...
        dims = len(lhs_out.shape)
        if dims == 1:
            matches = lhs_out == rhs_out
        else:
            matches = (lhs_out != rhs_out).sum(axis=tuple(range(1, dims))) <= allowable_deviation
